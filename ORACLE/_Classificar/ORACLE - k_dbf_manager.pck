create or replace package debug_proc.data_file_manager is
---------------------------------------------------------------------------------------------------
...
--  function split(p_list in varchar2, p_del in varchar2 default ',') return split_table pipelined;
  function split(p_list in clob, p_del in varchar2 default ',') return split_table pipelined;
...
/
create or replace package body debug_proc.data_file_manager is

---------------------------------------------------------------------------------------------------
-- Pacote que permite exportar informações e dados de uma tabela para arquivo DBF padrão DBsae III 
-- Criado por Luciano Merighetti Marwell - Parceiro da TRISCAL (CPF# Consultoria e Tecnologia LDTA)
---------------------------------------------------------------------------------------------------
...
--  function split(p_list in varchar2, p_del in varchar2 default ',') return split_table pipelined as
  function split(p_list in clob, p_del in varchar2 default ',') return split_table pipelined as
    l_idx  pls_integer;
    l_list varchar2(32767) := null; --trim(p_list);
  begin
    loop
      l_idx := instr(p_list, p_del);
      if l_idx > 0 then
        pipe row(substr(p_list, 1, l_idx - 1));
        l_list := substr(p_list, l_idx + length(p_del));
      else
        pipe row(l_list);
        exit;
      end if;
  ...
  begin
    ...
    if p_campos is not null then
      l_script := l_script || '     and column_name in(select trim(upper(column_value)) from table(debug.data_file_manager.split(' || chr(39) || p_campos || chr(39) || ')))' || crlf;
    end if;
